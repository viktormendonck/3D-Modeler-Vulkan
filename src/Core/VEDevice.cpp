...
VEDevice::VEDevice(VEWindow &window) : window{window} {
  createInstance();  //init vulkan lib and connects aplication to it
  setupDebugMessenger(); //setup debug messenger w/o this it would barely give any errors
  createSurface(); //create surface to draw on
  pickPhysicalDevice(); // pick a gpu
  createLogicalDevice(); // describes what features of device to use
  createCommandPool(); // helps w command buffer alocation
}
...
bool VEDevice::isDeviceSuitable(VkPhysicalDevice device) {
  ...
  //check if the current device is a discrete gpu
